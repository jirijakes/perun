...
enum Message:
  ...
  case ChannelAnnouncement(m: gossip.ChannelAnnouncement, spk: Option[String])
...
val messageCodec: Codec[Message] = discriminated[Message].by(uint16)
  ...
  .caseP(256) { case ChannelAnnouncement(m, _) => m }(ChannelAnnouncement.apply(_, None))(gossip.channelAnnouncement  )
